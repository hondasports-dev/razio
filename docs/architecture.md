...
第一面の構成は、上部のアンバー光、既存 Sans の `RAZIO` ワードマーク、円形の電源、単一の `LIVE` / `STANDBY`、プリセットと周波数カーブを収めた筐体パネル、細い出力メーター、Hiss / Crackle の筐体パネル、`詳細設定を開く` です。色は既存の night studio（アンバー＋チャコール）を維持し、設定アプリ風のスタジアムピルや巨大なアウトラインボタンは使いません。blurb直下の `（プリセット名）を初期値に戻す` は常時表示し、定義値から外れているときだけアンバーの活性表示で押せます。未変更時は非活性の輪郭だけにします。プリセットは `‹ ›`、点、名前／カーブ上の横スワイプのどれでも切り替わり、端の `Shortwave` / `Narrow AM` からも一周します。`session 0` や `DynamicsProcessing` の観測文言、6本の周波数スライダー、同調ダイヤルは詳細へ下げます。電源は既存の `onPowerChange` へ接続します。CRTテクスチャや端末ログ風のフッターは使いません。
...
実装上、6つの周波数境界は `詳細設定を開く` の中へ置きます。詳細を開いている間は第一面の Hiss / Crackle・出力メーター・ヒーローカーブを畳み、`詳細設定を閉じる` を全幅ボタンとして確保します。代わりに詳細パネル先頭のカーブをスライダーの直上へ置き、低域／高域カットの変更が同じ面で追従するようにします。詳細を閉じると第一面のカーブが戻ります。折りたたみ状態は `rememberSaveable(state.preset.id)` で保持し、プリセット切替で閉じます。
