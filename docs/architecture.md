...
第一面の構成は、上部のアンバー光、セリフの `RAZIO` ワードマーク、円形の電源、単一の `LIVE` / `STANDBY`、プリセットと周波数カーブを収めた筐体パネル、細い出力メーター、Hiss / Crackle の筐体パネル、`詳細設定を開く` です。色は既存の night studio（アンバー＋チャコール）を維持し、設定アプリ風のスタジアムピルや巨大なアウトラインボタンは使いません。blurb直下の `（プリセット名）を初期値に戻す` は常時表示し、定義値から外れているときだけアンバーの活性表示で押せます。未変更時は非活性の輪郭だけにします。プリセットは `‹ ›`、点、名前／カーブ上の横スワイプのどれでも切り替わります。`session 0` や `DynamicsProcessing` の観測文言、6本の周波数スライダー、同調ダイヤルは詳細へ下げます。電源は既存の `onPowerChange` へ接続します。CRTテクスチャや端末ログ風のフッターは使いません。
