...
`RazioHomeScreen` は電源・処理方式／プリセット・周波数カーブ・詳細調整・ノイズ・エンジン状態を独立した `RetroPanel` に分けます。`RetroPanel` は caller が配置できる `modifier` と `ColumnScope` の content slot を持ち、端末コンソールの角丸を抑えた枠・内側余白だけを所有します。プリセットは `LazyRow` で横スクロールさせ、端末幅が狭くても `Narrow AM` / `Vintage speaker` などのラベルを縦方向へ潰しません。ヘッダーには電源スイッチと状態LEDを置き、既存の電源操作経路を変えずに端末モニターのON/OFF表示へまとめます。
...
プリセット選択パネルの下には周波数カーブを常時表示し、その下の `DETAILS / 開く` ボタンで選択中プリセットをその場で試せる開発用調整値を展開します。隣の `プリセット初期値に戻す` は折りたたみ中でも利用でき、選択中プリセットの定義値を再適用します。UIはAndroid標準のCompose `Slider`で、周波数6点（低域カット開始／低域カット中間／中域開始／中域終了／高域カット中間／高域カット開始）には一定刻みの`−` / `＋`ボタンも併設します。低域・高域の中間ゲインも個別に変更でき、ゲイン、MBC、入力ゲイン、歪み緩和、Fading深度・周期も同じ詳細パネルから変更できます。

詳細パネルには `PresetFrequencyCurve` と同じモデルを使った6本の周波数境界スライダーを配置し、従来の`tuning dial`もその直前に残します。カーブ本体は詳細を閉じても残し、20 Hz〜20 kHzを対数軸にした細分化グリッド上へ、`AudioPresetTuning` の5つのゲイン点を補間したカーブを描き、低域／高域のカット帯と中域帯を網掛け、6つの周波数境界を縦線で示します。スライダーや`−` / `＋`で確定した値に追従するため、どの帯域を削っているかを数値だけでなく視覚的に確認できます。カーブは調整モデルから算出した設計目安で、端末のnative effect出力を直接測定するスペクトラムではありません。
