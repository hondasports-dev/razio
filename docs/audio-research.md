...
- Double playback finding: PoCは元アプリのAudioTrackを所有せず、ミュート・duck・差し替えを行わない。したがって上記の同時`started`状態は「元音声 + 加工音」の二重再生が起こり得ることの実機Evidenceであり、二重再生を防げたとは判定しない。RAZIO側はAudioFocusを要求していないため、radikoの再生状態は維持された。人の聴感による音量差／エコーの定量評価は今回未実施で、製品採用条件には含めない
...
- Status: **Pixel 10 Proで2ch入力形式のcapture→downmix→AudioTrack再生→停止ループが動作した**。これは左右独立信号や音質を証明する合格ではない。元音声の抑制はできず、二重再生リスクとbuffer依存の概算遅延（約170–506 ms）、usage全体取得（UID指定なし）が残るため、現行製品backendはDynamicsProcessing global経路のまま、ロードマップのMono製品採用チェックは未完了とする
